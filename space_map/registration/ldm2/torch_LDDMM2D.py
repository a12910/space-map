...
#             indexing='ij'
#         )
#         self.grid = torch.stack([x - x.mean(), y - y.mean()]).float()
        
#     def forward_deformation(self, source: torch.Tensor, v: torch.Tensor) -> torch.Tensor:
#         """前向变形"""
#         phi = self.grid.clone()
        
#         for t in range(self.nt):
#             # 计算速度场
#             v_t = compute_velocity_field(v[t], phi, self.dt)
            
#             # 应用变形
#             phi = apply_deformation(
#                 phi - self.grid, v_t, 
#                 self.nx, self.dx, mode='border'
#             ) + v_t
            
#             # 应用仿射变换
#             if t == self.nt-1 and self.do_affine:
#                 phi = self.apply_affine(phi)
                
#         return phi
    
#     def apply_affine(self, phi: torch.Tensor) -> torch.Tensor:
#         """应用仿射变换"""
#         if not hasattr(self, 'affine_matrix'):
#             return phi
            
#         coords_homo = torch.cat([phi, torch.ones_like(phi[0:1])], dim=0)
#         transformed = torch.matmul(
#             self.affine_matrix, 
#             coords_homo.reshape(3, -1)
#         ).reshape(phi.shape)
        
#         return transformed[0:2]
    
#     def calculate_gradient_affine(self, lambda1: torch.Tensor, 
#                                 deformed: torch.Tensor,
#                                 mode: str = 'affine') -> torch.Tensor:
#         """计算仿射变换的梯度"""
#         # 计算图像梯度
#         grad_x, grad_y = self.calculate_image_gradient(deformed)
        
#         # 计算仿射矩阵梯度
#         grad_A = torch.zeros((3,3), device=self.device)
#         for r in range(2):
#             for c in range(3):
#                 dA = torch.zeros((3,3), device=self.device)
#                 dA[r,c] = 1.0
#                 AdAB = torch.matmul(
#                     torch.matmul(self.affine_matrix, dA),
#                     torch.inverse(self.affine_matrix)
#                 )
                
#                 grad_term = (
#                     grad_x * (AdAB[0,0]*self.grid[0] + AdAB[0,1]*self.grid[1] + AdAB[0,2]) +
#                     grad_y * (AdAB[1,0]*self.grid[0] + AdAB[1,1]*self.grid[1] + AdAB[1,2])
#                 )
                
#                 grad_A[r,c] = torch.sum(lambda1 * grad_term) * self.dx[0] * self.dx[1]
        
#         # 处理特殊模式
#         if mode == 'rigid':
#             grad_A = grad_A - grad_A.t()
#         elif mode == 'sim':
#             grad_A_t = grad_A.t().clone()
#             grad_A_t[0,0] = grad_A_t[1,1] = 0
#             grad_A = grad_A - grad_A_t
...
#             self.initialize_grid(source.shape[-2:])
            
#         # 计算变形场
#         phi = self.forward_deformation(source, self.velocity)
        
#         # 应用变形得到结果
#         deformed = apply_deformation(source, phi, self.nx, self.dx)
        
#         # 计算损失
#         sim_loss = F.mse_loss(deformed, target)
#         reg_loss = self.calculate_regularization(self.velocity)
        
#         return deformed, sim_loss + reg_loss
...
#             if t == self.params['nt']-1 and (self.params['do_affine'] > 0  or \
...
#                                                                           (self.nx[1]*self.dx[1]-self.dx[1])*2,
#                                                                           phiinv0_temp/
#                                                                         (self.nx[0]*self.dx[0]-self.dx[0])*2),padding_mode='zeros'))
...
#                                                                           phiinv0_temp/(self.nx[0]*self.dx[0]-self.dx[0])*2),padding_mode='zeros'))
