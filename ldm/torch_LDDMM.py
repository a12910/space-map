...
class LDDMM(base.LDDMMBase):
    ...
    def initializeVariables(self):
        ...
        if self.params['v_scale'] < 1.0:
            ...
            self.gaussian_filter = torch.tensor(base.mygaussian3d(sigma=1.0/self.params['v_scale'],size=size)).type(self.params['dtype']).to(device=self.params['cuda'])
        ...
        if hasattr(self, 'vt0') and self.initializer_flags['v_scale'] == 1:
            ...
            for i in range(self.params['nt']):
                self.vt0[i] = torch.squeeze(base.grid_sample((self.vt0[i]).unsqueeze(0).unsqueeze(0),torch.stack(((self.X2)/(self.nx[2]*self.dx[2]-self.dx[2])*2,(self.X1)/(self.nx[1]*self.dx[1]-self.dx[1])*2,(self.X0)/(self.nx[0]*self.dx[0]-self.dx[0])*2),dim=3).unsqueeze(0),padding_mode='border'))
                self.vt1[i] = torch.squeeze(base.grid_sample((self.vt1[i]).unsqueeze(0).unsqueeze(0),torch.stack(((self.X2)/(self.nx[2]*self.dx[2]-self.dx[2])*2,(self.X1)/(self.nx[1]*self.dx[1]-self.dx[1])*2,(self.X0)/(self.nx[0]*self.dx[0]-self.dx[0])*2),dim=3).unsqueeze(0),padding_mode='border'))
                self.vt2[i] = torch.squeeze(base.grid_sample((self.vt2[i]).unsqueeze(0).unsqueeze(0),torch.stack(((self.X2)/(self.nx[2]*self.dx[2]-self.dx[2])*2,(self.X1)/(self.nx[1]*self.dx[1]-self.dx[1])*2,(self.X0)/(self.nx[0]*self.dx[0]-self.dx[0])*2),dim=3).unsqueeze(0),padding_mode='border'))
    ...
    def forwardDeformation(self):
        ...
        for t in range(self.params['nt']):
            ...
            if self.params['do_lddmm'] == 1 or hasattr(self, 'vt0'):
                phiinv0_gpu = torch.squeeze(base.grid_sample((phiinv0_gpu-self.X0).unsqueeze(0).unsqueeze(0),torch.stack(((self.X2-self.vt2[t]*self.dt)/(self.nx[2]*self.dx[2]-self.dx[2])*2,(self.X1-self.vt1[t]*self.dt)/(self.nx[1]*self.dx[1]-self.dx[1])*2,(self.X0-self.vt0[t]*self.dt)/(self.nx[0]*self.dx[0]-self.dx[0])*2),dim=3).unsqueeze(0),padding_mode='border')) + (self.X0-self.vt0[t]*self.dt)
                phiinv1_gpu = torch.squeeze(base.grid_sample((phiinv1_gpu-self.X1).unsqueeze(0).unsqueeze(0),torch.stack(((self.X2-self.vt2[t]*self.dt)/(self.nx[2]*self.dx[2]-self.dx[2])*2,(self.X1-self.vt1[t]*self.dt)/(self.nx[1]*self.dx[1]-self.dx[1])*2,(self.X0-self.vt0[t]*self.dt)/(self.nx[0]*self.dx[0]-self.dx[0])*2),dim=3).unsqueeze(0),padding_mode='border')) + (self.X1-self.vt1[t]*self.dt)
                phiinv2_gpu = torch.squeeze(base.grid_sample((phiinv2_gpu-self.X2).unsqueeze(0).unsqueeze(0),torch.stack(((self.X2-self.vt2[t]*self.dt)/(self.nx[2]*self.dx[2]-self.dx[2])*2,(self.X1-self.vt1[t]*self.dt)/(self.nx[1]*self.dx[1]-self.dx[1])*2,(self.X0-self.vt0[t]*self.dt)/(self.nx[0]*self.dx[0]-self.dx[0])*2),dim=3).unsqueeze(0),padding_mode='border')) + (self.X2-self.vt2[t]*self.dt)
    ...
    def applyThisTransformPoints(self, points: np.array, interpmode='bilinear',dtype='torch.FloatTensor'):
        ...
        for t in range(self.params['nt']):
            ...
            if self.params['v_scale'] != 1.0:
                ph1 = torch.stack((
                    torch.squeeze(interpolate(phiinv2_gpu.unsqueeze(0).unsqueeze(0),
                                                size=(self.nx[0],self.nx[1],self.nx[2]),
                                                mode='trilinear',
                                                align_corners=True))
                    .type(dtype)
                    .to(device=self.params['cuda'])/(self.nx[2]*self.dx[2]-self.dx[2])*2,
                ...
                ph1 = torch.stack((phiinv2_gpu.type(dtype).to(device=self.params['cuda'])/(self.nx[2]*self.dx[2]-self.dx[2])*2,
    ...
    def calculateGradientA(self,affineA,lambda1,mode='affine'):
        ...
        # if rigid
        if mode == 'rigid':
            self.gradA -= torch.transpose(self.gradA.clone(),0,1)
